...
(function(exports) {
  ...
  WifiP2pManager.prototype = {
    ...
    onPeerClicked: function(aPeerInfo) {
      debug('Peer: ' + aPeerInfo.address + ' is clicked: ' + aPeerInfo.connectionStatus);
    ...
    handlePairingRequest: function(aEvent) {
      ...
      switch (aEvent.wpsMethod) {
        ...
        case "display":
          ...
          setPairingConfirm(true, aEvent.pin); // !!! Confirm before alert() to avoid bugs
          break;
        case "keypad":
          var pin = prompt("PIN");
          if (pin) {
    ...
    onstatuschange: function(aEvent) {
      ...
      if (groupOwner) {
        ...
        if (CONNECT_WFD && this.manager.listenForRemoteDisplay) {
          this.manager.listenForRemoteDisplay();
        }
      }

      debug('The peer whose status has just changed is: ' + aEvent.peerAddress);
